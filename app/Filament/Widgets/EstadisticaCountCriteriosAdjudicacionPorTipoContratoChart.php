...
class EstadisticaCountCriteriosAdjudicacionPorTipoContratoChart extends \App\Filament\Widgets\BaseChartWidget
{
...
//    protected ?string $maxHeight = '200px';
    ...
    protected function getData(): array
    {
        return $this->buildCountDataset(
            \App\Models\PLACSP\CriterioAdjudicacion::class,
            'awarding_criteria_type_code',
            fn(string $c) => \App\Enums\Placsp\PLACSP_AwardingCriteriaCode::tryFrom($c)?->getLabel() ?? $c,
            'Total Criterios de adjudicación'
        );
    }
    ...
    protected function getDataOld(): array
    {
        ...
        // Base query
        $queryTotales = CriterioAdjudicacion::query();

        $service = new EstadisticasService();

        $estadisticas = $service->getEstadisticasCountPorCampo($queryTotales,'awarding_criteria_type_code');
        ...
        foreach ($estadisticas as $codigo => $total) {
            $enum = PLACSP_AwardingCriteriaCode::tryFrom((string) $codigo);
